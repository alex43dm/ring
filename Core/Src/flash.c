...
uint32_t flash_write(uint32_t *buf, uint16_t n)
{
    uint32_t error;
    uint32_t address;
    ...
    if (HAL_FLASHEx_Erase(&es, &error) != HAL_OK)
    {
        return HAL_FLASH_GetError();
    }

    for(address = PAGE_127_ADDRESS; n != 0 && IS_PAGE_END(address); n--, buf++, address += 4)
    {
...
void flash_read(uint32_t *buf, uint16_t n)
{
    uint32_t address;
